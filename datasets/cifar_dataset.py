...
from ffcv.transforms import ToTensor, ToDevice, ToTorchImage, RandomTranslate, RandomHorizontalFlip, Convert, ModuleWrapper
...
def create_dataloader(path, configs):
    ...
    augment_pipeline = [decoder, 
                      RandomTranslate(padding = 4), 
                      RandomHorizontalFlip(),
                      ToTensor(),
                      ToTorchImage()]
    if configs["augmentations"]["brightness"] != 0.0 and configs["augmentations"]["hue"] != 0.0 and configs["augmentations"]["contrast"] != 0.0 and configs["augmentations"]["saturation"] != 0.0:
    ...
    loader = Loader(path, batch_size=configs["experiment"]["general"]["bz"], num_workers=8,
...
def cifar_wrapper(config, train=0):
    if train == 0 or train == 1:
        return torchvision.datasets.CIFAR100(root=config["meta"]["dataset_path"], train=True, download=True)
    elif train == 2:
        return torchvision.datasets.CIFAR100(root=config["meta"]["dataset_path"], train=False, download=True)
