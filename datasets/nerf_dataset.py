...
class Nerf_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rays_o, rays_d = self.get_rays(
            self.size, self.size, self.focal, self.extrinsics[index]
        )
    ...
    def extrinsic_handler(self, file_name):
        ...
        extrinsic = torch.tensor(extrinsic, dtype=torch.float32, device=device).reshape(
            4, 4
        )
        return extrinsic
