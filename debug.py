...
def positional_encoding(x):
    ...
    coeffs_pose = (torch.pi*torch.pow((torch.ones(pose_enc_len,device=device)*2), indices_x_pose))
    coeffs_dir = torch.pi*torch.pow((torch.ones(dir_enc_len,device=device)*2), indices_x_dir)
    sin_pos = torch.sin(coeffs_pose*x_position)
    cos_pos = torch.cos(coeffs_pose*x_position)
    pose_enc = torch.stack((sin_pos, cos_pos), dim=0).view(x.shape[0], 2*pose_enc_len).t().contiguous().view(x.shape[0], 2*self.pose_enc_len)
    sin_dir = torch.sin(coeffs_dir*x_dir)
    cos_dir = torch.cos(coeffs_dir*x_dir)
    dir_enc = torch.stack((sin_dir, cos_dir), dim=0).view(x.shape[0], 2*dir_enc_len).t().contiguous().view(x.shape[0], 2*self.dir_enc_len)
