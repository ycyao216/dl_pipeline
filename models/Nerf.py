...
class Nerf_block(nn.Module):
    ...
    def positional_encoding(self, x):
        ...
        x_position = x[:, :3]
        x_dir = x[:, 3:]
        indices_x_pose = torch.arange(
            0, self.pose_enc_len, dtype=torch.int, device=device
        )
        indices_x_dir = torch.arange(
            0, self.dir_enc_len, dtype=torch.int, device=device
        )
        coeffs_pose = torch.pi * torch.pow(
            (torch.ones(self.pose_enc_len, device=device) * 2), indices_x_pose
        )
        coeffs_dir = torch.pi * torch.pow(
            (torch.ones(self.pose_enc_len, device=device) * 2), indices_x_dir
        )
    ...
    def posenc(self, x):
        ...
        for i in range(self.pose_enc_len):
            for fn in [torch.sin, torch.cos]:
                pose_enc.append(fn(2.0**i * torch.pi * pos_x))
                if i < self.dir_enc_len:
                    dir_enc.append(fn(2.0**i * torch.pi * dir_x))
